...
async fn test_client_operations() {
    ...
    let (mut worker, mut client, subscription_manager) =
    ...
    tokio::select! {
        Some(captured_response) = client.recv() => {
            match captured_response.updates.first() {
                Some(XdsResourceUpdate::Update(name, _payload, _version)) => {
...
async fn test_client_resilience() {
    ...
    let (mut worker, mut client, subscription_manager) =
        DiscoveryClientBuilder::<bindings::ClusterDiscoveryType>::new(node, typed_binding)
...
async fn test_aggregated_discovery() {
    ...
    let client = DiscoveryClientBuilder::new(node.clone(), typed_binding)
    ...
    let (mut worker, mut client, subscription_manager) =
        DiscoveryClientBuilder::new(node, typed_binding).build().unwrap();

    let _ = subscription_manager.subscribe("cluster-a".to_owned(), TypeUrl::Cluster).await;
    let _ = subscription_manager.subscribe("cluster-z".to_owned(), TypeUrl::Cluster).await;
    let _ = subscription_manager.subscribe("endpoints-a".to_owned(), TypeUrl::ClusterLoadAssignment).await;
    let _ = subscription_manager.subscribe("secret-config-a".to_owned(), TypeUrl::Secret).await;
