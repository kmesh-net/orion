...
//!
//! This filter sets filter state values based on request/response headers.
//! Commonly used in Istio for setting dynamic metadata like connect_authority.
//!
//! Type URL: `type.googleapis.com/envoy.extensions.filters.http.set_filter_state.v3.Config`
...
/// Configuration for Envoy's set_filter_state filter
///
/// This filter sets filter state objects based on HTTP headers or other
/// request/response properties. The state can be accessed by other filters
/// or used in logging/telemetry.
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct SetFilterStateConfig {
    /// Actions to perform on request headers
    #[serde(skip_serializing_if = "Option::is_none")]
    pub on_request_headers: Option<Vec<FilterStateAction>>,

    /// Actions to perform on response headers
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Action to set a filter state value
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct FilterStateAction {
    /// The key to set in filter state
...
/// Format string configuration for extracting values
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct FormatString {
    /// Format string with substitution tokens
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Text format source for substitution
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct TextFormatSource {
    /// Inline format string (e.g., "%REQ(HOST)%")
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    fn from_json_value(value: JsonValue) -> Result<Self, GenericError> {
        serde_json::from_value(value).map_err(|e| {
            GenericError::from_msg_with_cause(
                "Failed to deserialize SetFilterStateConfig from JSON",
                e,
            )
        })
    }
...
mod tests {
    ...
    fn test_deserialize_connect_authority_config() {
        // This is the typical Istio waypoint configuration for connect_authority
        let json = serde_json::json!({
            "on_request_headers": [
                {
    ...
    fn test_from_typed_struct() {
        // Create a protobuf Struct with set_filter_state config
        let mut action_fields = BTreeMap::new();
        action_fields.insert(
            "object_key".to_string(),
            Value {
                kind: Some(Kind::StringValue("test_key".to_string())),
            },
        );
        ...
        let typed_struct = TypedStruct {
            type_url: SetFilterStateConfig::TYPE_URL.to_string(),
