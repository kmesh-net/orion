    ...
    impl TryFrom<EnvoyCluster> for Cluster {
        ...
        fn try_from(envoy: EnvoyCluster) -> Result<Self, Self::Error> {
            ...
            unsupported_field!(
                ...
                common_http_protocol_options,
                http_protocol_options,
                // Support http2_protocol_options: if present, set http_protocol_options to HTTP/2
                // http2_protocol_options,
                // typed_extension_protocol_options,
                dns_refresh_rate,
                dns_failure_refresh_rate,
                ...
                track_cluster_stats,
                preconnect_policy,
                connection_pool_per_downstream_connection // cluster_discovery_type,
                                                          // lb_config
            )?;
            ...
            let mut http_protocol_options = http_protocol_options.into_iter().next().unwrap_or_default();
            if http2_protocol_options.is_some() {
                http_protocol_options.codec = crate::config::cluster::http_protocol_options::Codec::Http2;
            }
