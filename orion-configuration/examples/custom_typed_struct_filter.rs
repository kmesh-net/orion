...
fn main() -> Result<(), GenericError> {
    println!("🔌 Custom TypedStruct Filter Plugin Example\n");

    println!("1. Creating custom filter parser...");
    let parser = GenericFilterParser::<CustomMetricsFilter>::new(CustomMetricsFilter::TYPE_URL);

    println!("2. Registering filter with global registry...");
    let registry = global_registry();
    registry.register_dynamic(parser)?;

    println!("3. Verifying registration...");
    assert!(registry.is_supported(CustomMetricsFilter::TYPE_URL));
    println!("   ✓ Filter registered successfully");
    ...
    let typed_struct = ParsedTypedStruct { type_url: CustomMetricsFilter::TYPE_URL.to_string(), value: config_json };
    ...
    println!("   ✓ Filter parsed successfully:");
    println!("     - Enabled: {}", filter.enabled);
    println!("     - Endpoint: {}", filter.metrics_endpoint.as_ref().unwrap());
    println!("     - Sample Rate: {}", filter.sample_rate.unwrap());
    println!("     - Labels: {} entries", filter.labels.as_ref().unwrap().len());

    println!("\n5. Listing all registered filters:");
    let registered = registry.supported_types();
    println!("   Total registered filters: {}", registered.len());
    for (i, type_url) in registered.iter().enumerate() {
        println!("   {}. {}", i + 1, type_url);
    }
