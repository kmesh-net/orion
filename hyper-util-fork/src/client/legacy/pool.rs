...
impl EventHandler {
    ...
    {
        EventHandler { on_event: Arc::new(update), tag: Arc::new(tag) }
    }
        ...
        if ver == Ver::Http2 && can_share {
            if let Some(ref enabled) = self.inner {
                ...
                return if inner.connecting.insert(key.clone()) {
                    let connecting = Connecting { key: key.clone(), pool: WeakOpt::downgrade(enabled), can_share };
                    Some(connecting)
                } else {
        ...
        while let Some(entry) = self.list.pop() {
            ...
            let value = match entry.value.reserve() {
                ...
                Reservation::Shared(to_reinsert, to_checkout) => {
                    self.list.push(Idle { idle_at: now, value: to_reinsert });
                    to_checkout
                },
...
mod tests {
    ...
    fn c<T: Poolable, K: Key>(key: K) -> Connecting<T, K> {
        Connecting { key, pool: WeakOpt::none(), can_share: true }
    }
