...
pub(crate) fn get_server_prefix(server: &OpenAPI) -> Result<String, ParseError> {
    match server.servers.len() {
        ...
        _ => Err(ParseError::UnsupportedReference(format!("multiple servers are not supported: {:?}", server.servers))),
    }
}
...
fn resolve_schema<'a>(reference: &'a ReferenceOr<Schema>, doc: &'a OpenAPI) -> Result<&'a Schema, ParseError> {
    match reference {
        ReferenceOr::Reference { reference } => {
            ...
            let schema =
                components.schemas.get(reference).ok_or(ParseError::MissingReference(reference.to_string()))?;
...
fn resolve_nested_schema<'a>(reference: &'a ReferenceOr<Schema>, doc: &'a OpenAPI) -> Result<Schema, ParseError> {
...
fn resolve_parameter<'a>(reference: &'a ReferenceOr<Parameter>, doc: &'a OpenAPI) -> Result<&'a Parameter, ParseError> {
    match reference {
        ReferenceOr::Reference { reference } => {
            ...
            let components: &openapiv3::Components = doc.components.as_ref().ok_or(ParseError::MissingComponents)?;
            let parameter =
                components.parameters.get(reference).ok_or(ParseError::MissingReference(reference.to_string()))?;
...
fn resolve_request_body<'a>(
    ...
    match reference {
        ReferenceOr::Reference { reference } => {
            ...
            let components: &openapiv3::Components = doc.components.as_ref().ok_or(ParseError::MissingComponents)?;
            let request_body =
                components.request_bodies.get(reference).ok_or(ParseError::MissingReference(reference.to_string()))?;
...
pub(crate) fn parse_openapi_schema(open_api: &OpenAPI) -> Result<Vec<(Tool, UpstreamOpenAPICall)>, ParseError> {
            ...
            let items: Result<Vec<_>, _> = item
                ...
                .map(|(method, op)| -> Result<(Tool, UpstreamOpenAPICall), ParseError> {
                    let name = op
                        .operation_id
                        .clone()
                        .ok_or(ParseError::InformationRequired(format!("operation_id is required for {path}")))?;
                    ...
                    op.parameters.iter().try_for_each(|p| -> Result<(), ParseError> {
                    ...
                    for (param_type, props) in param_schemas {
                        let sub_schema = JsonSchema {
                            required: props
                                ...
                                .collect(),
                            properties: props.iter().map(|(name, s, _)| (name.clone(), json!(s))).collect(),
                            ..Default::default()
                        };
                    ...
                    let final_json = final_json
                        .as_object()
                        .ok_or(ParseError::UnsupportedReference("final schema is not an object".to_string()))?
                        .clone();
                    let tool = Tool {
                        annotations: None,
                        name: Cow::Owned(name.clone()),
                        description: Some(Cow::Owned(
                            op.description.as_ref().unwrap_or_else(|| op.summary.as_ref().unwrap_or(&name)).to_string(),
                        )),
...
impl Default for JsonSchema {
    fn default() -> Self {
        Self { required: vec![], properties: JsonObject::new(), r#type: "object".to_string() }
    }
}
...
impl Handler {
    pub async fn send_message(
        ...
        let res = match request.request {
            ...
            ClientRequest::GetPromptRequest(_) => {
                Messages::from_result(id, GetPromptResult { description: None, messages: vec![] })
            },
            ClientRequest::ListPromptsRequest(_) => {
                Messages::from_result(id, ListPromptsResult { next_cursor: None, prompts: vec![] })
            },
            ClientRequest::ListResourcesRequest(_) => {
                Messages::from_result(id, ListResourcesResult { next_cursor: None, resources: vec![] })
            },
            ClientRequest::ListResourceTemplatesRequest(_) => {
                Messages::from_result(id, ListResourceTemplatesResult { next_cursor: None, resource_templates: vec![] })
            },
            ClientRequest::ReadResourceRequest(_) => Messages::from_result(id, ReadResourceResult { contents: vec![] }),
            ...
            ClientRequest::CallToolRequest(ctr) => {
                let res = self.call_tool(ctr.params.name.as_ref(), ctr.params.arguments, ctx).await?;
                Messages::from_result(
                    id,
                    CallToolResult { content: vec![], structured_content: Some(res), is_error: None, meta: None },
                )
            },
            ClientRequest::ListToolsRequest(_) => {
                Messages::from_result(id, ListToolsResult { next_cursor: None, tools: self.tools() })
            },
        };
        Ok(res)
    }
    ...
    pub async fn call_tool(
        ...
        let base_url = format!("{}://{}{}", "http", self.backend.hostport(), normalized_path);
        ...
        let query_string = if !query_params.is_empty() {
            ...
            if !pairs.is_empty() { format!("?{}", pairs.join("&")) } else { String::new() }
        } else {
            String::new()
        };
        ...
        // Make the request
        let response =
            self.client.call_with_default_policies(request, &self.backend, self.default_policies.clone()).await?;
