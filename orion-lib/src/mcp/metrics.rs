...
impl Metrics {
    pub fn new(registry: &mut Registry, _additional_tags: Option<HashMap<String, String>>) -> Self {
        ...
        registry.register("tool_calls", "The total number of tool calls", tool_calls.clone());

        let tool_call_errors = Family::default();
        registry.register("tool_call_errors", "The total number of tool call errors", tool_call_errors.clone());

        let list_calls = Family::default();
        registry.register("list_calls", "The total number of list calls", list_calls.clone());
        ...
        registry.register("get_prompt_calls", "The total number of get prompt calls", get_prompt_calls.clone());

        Self { tool_calls, tool_call_errors, list_calls, read_resource_calls, get_prompt_calls }
    }
...
impl Recorder<GetResourceCall, ()> for Metrics {
    fn record(&self, mut get_resource_call: GetResourceCall, _: ()) {
        ...
        self.read_resource_calls.get_or_create(&get_resource_call).inc();
    }
}
