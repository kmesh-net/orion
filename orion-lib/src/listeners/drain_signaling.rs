...
impl DrainScenario {
    pub fn should_drain(self, drain_type: ConfigDrainType) -> bool {
        match (self, drain_type) {
            (_, ConfigDrainType::Default) => true,
            (DrainScenario::ListenerUpdate, ConfigDrainType::ModifyOnly) => true,
            (DrainScenario::HotRestart, ConfigDrainType::ModifyOnly) => true,
...
impl ListenerDrainContext {
    ...
    pub fn is_timeout_exceeded(&self) -> bool {
        let global_timeout = match &self.strategy {
            DrainStrategy::Tcp { global_timeout } => *global_timeout,
            DrainStrategy::Http { global_timeout, .. } => *global_timeout,
...
impl DrainSignalingManager {
    ...
    pub async fn start_listener_draining(&self, drain_state: ListenerDrainState) {
        if !Self::should_drain(drain_state.drain_scenario, drain_state.drain_type) {
            debug!(
    ...
    pub fn apply_http1_drain_signal_sync<B>(&self, response: &mut hyper::Response<B>) -> bool {
        use hyper::header::{HeaderValue, CONNECTION};
        response.headers_mut().insert(CONNECTION, HeaderValue::from_static("close"));
        debug!("Applied 'Connection: close' header for HTTP/1.1 drain signaling");
        true
    ...
    pub async fn initiate_listener_drain(
        ...
        tokio::spawn(async move {
            let _ = manager_clone.monitor_drain_progress(context_clone, listener_id_clone).await;
        });
