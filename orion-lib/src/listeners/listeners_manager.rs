...
impl ListenerInfo {
    ...
    fn get_drain_progress(&self, connection_manager: &DefaultConnectionManager) -> DrainProgress {
        let (percentage, elapsed, remaining_time) =
            if let ListenerState::Draining { started_at, drain_config } = &self.state {
                let elapsed = started_at.elapsed();
                let percentage = Self::calculate_drain_percentage(elapsed, drain_config);
        ...
        let all_connections = connection_manager.get_listener_connections(&self.listener_conf.name);
    ...
    fn estimate_drain_completion(&self, connection_manager: &DefaultConnectionManager) -> Option<Instant> {
        if let ListenerState::Draining { started_at, drain_config } = &self.state {
            let current_connections = self.connections_count.load(Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_config_change_drains_old_listener() {
        ...
        man.start_listener(listener1, config1.clone()).await.unwrap();
    ...
    async fn test_address_conflict_resolution_graceful() {
        ...
        man.start_listener_with_conflict_resolution(l1, l1_info.clone()).await.unwrap();
    ...
    async fn test_concurrent_listener_operations() {
        ...
        // Verify that all listeners were created successfully
