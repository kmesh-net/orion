...
use crate::http::{filters, DropBody, Request, Response};
...
impl LegacySSEService {
    ...
    pub async fn handle_post(&self, request: Request) -> Response {
        ...
        let message = match json::from_body::<ClientJsonRpcMessage>(body).await {
            Ok(b) => b,
            Err(e) => {
                return http_error(StatusCode::BAD_REQUEST, format!("fail to deserialize request body: {e}"));
            },
        };
        ...
        if is_init {
            ...
            if let Err(e) = session.forward_legacy_sse(get_stream).await {
                return http_error(StatusCode::INTERNAL_SERVER_ERROR, format!("fail to establish get stream: {e}"));
            }
        }
        if let Err(e) = session.forward_legacy_sse(resp).await {
            return http_error(StatusCode::INTERNAL_SERVER_ERROR, format!("fail to send message: {e}"));
        }
        accepted_response()
    }
    ...
    pub async fn handle_get(&self, request: Request) -> Response {
        let relay = match (self.service_factory)() {
            Ok(r) => r,
            Err(e) => {
                return http_error(StatusCode::INTERNAL_SERVER_ERROR, format!("fail to create relay: {e}"));
            },
        };
        ...
        if let Err(e) = http::modify_url(&mut base_url, |url| {
            ...
            return http_error(StatusCode::INTERNAL_SERVER_ERROR, format!("fail to create SSE url: {e}"));
        }
        ...
        Sse::new(stream)
            .into_response()
            .map(|b| http::Body::new(DropBody::new(b, session::dropper(self.session_manager.clone(), session, parts))))
    }
}
...
fn accepted_response() -> Response {
    ::http::Response::builder().status(StatusCode::ACCEPTED).body(crate::http::Body::empty()).expect("valid response")
}
