...
impl Session {
    ...
    pub async fn send(&self, parts: Parts, message: ClientJsonRpcMessage) -> Response {
        ...
        self.send_internal(parts, message).await.unwrap_or_else(Self::handle_error(req_id))
    }
    ...
    pub async fn stateless_send_and_initialize(&self, parts: Parts, message: ClientJsonRpcMessage) -> Response {
    ...
    pub async fn delete_session(&self, parts: Parts) -> Response {
        ...
        self.relay.send_fanout_deletion(ctx).await.unwrap_or_else(Self::handle_error(None))
    }
    ...
    pub async fn forward_legacy_sse(&self, resp: Response) -> Result<(), ClientError> {
        ...
        let sse = match content_type {
            Some(ct) if ct.as_bytes().starts_with(EVENT_STREAM_MIME_TYPE.as_bytes()) => {
                ...
                let event_stream = match resp.into_body() {
                    PolyBody::Empty(empty) => SseStream::from_byte_stream(empty.into_data_stream()).boxed(),
                    PolyBody::Full(full) => SseStream::from_byte_stream(full.into_data_stream()).boxed(),
                    PolyBody::Incoming(incoming) => SseStream::from_byte_stream(incoming.into_data_stream()).boxed(),
                    PolyBody::Timeout(body_with_timeout) => {
                        SseStream::from_byte_stream(body_with_timeout.into_data_stream()).boxed()
                    },
                    PolyBody::Grpc(unsync_box_body) => {
                        SseStream::from_byte_stream(unsync_box_body.into_data_stream()).boxed()
                    },
                    PolyBody::Stream(unsync_box_body) => {
                        SseStream::from_byte_stream(unsync_box_body.into_data_stream()).boxed()
                    },
                };
    ...
    pub async fn get_stream(&self, parts: Parts) -> Response {
        let ctx = IncomingRequestContext::new(parts);
        self.relay.send_fanout_get(ctx).await.unwrap_or_else(Self::handle_error(None))
    }
    ...
    fn handle_error(req_id: Option<RequestId>) -> impl FnOnce(UpstreamError) -> Response {
        move |e| {
            ...
            http_error(StatusCode::INTERNAL_SERVER_ERROR, err.unwrap_or_else(|| format!("failed to send message: {e}")))
        }
    }

    async fn send_internal(&self, parts: Parts, message: ClientJsonRpcMessage) -> Result<Response, UpstreamError> {
            ...
            ClientJsonRpcMessage::Request(mut r) => {
                ...
                match &mut r.request {
                    ClientRequest::InitializeRequest(_) => {
                        self.relay.send_fanout(r, ctx, self.relay.merge_initialize()).await
                    },
                    ClientRequest::ListToolsRequest(_) => {
                        self.relay.send_fanout(r, ctx, self.relay.merge_tools()).await
                    },
                    ClientRequest::PingRequest(_) | ClientRequest::SetLevelRequest(_) => {
                        self.relay.send_fanout(r, ctx, self.relay.merge_empty()).await
                    },
...
impl SessionManager {
    ...
    pub fn create_session(&self, relay: Relay) -> Session {
        let id = session_id();
        let sess = Session { id: id.clone(), relay: Arc::new(relay), tx: None };
    ...
    pub fn create_legacy_session(&self, relay: Relay) -> (Session, Receiver<ServerJsonRpcMessage>) {
        ...
        let id = session_id();
        let sess = Session { id: id.clone(), relay: Arc::new(relay), tx: Some(tx) };
        let mut sm = self.sessions.write().expect("write lock");
        sm.insert(id.to_string(), sess.clone());
        (sess, rx)
    }
...
pub fn dropper(sm: Arc<SessionManager>, s: Session, parts: Parts) -> SessionDropper {
    SessionDropper { sm, s: Some((s, parts)) }
}
...
fn http_error(status: StatusCode, body: impl Into<PolyBody>) -> Response {
...
impl sse_stream::Timer for TokioSseTimer {
    fn from_duration(duration: Duration) -> Self {
        Self { sleep: tokio::time::sleep(duration) }
    }
