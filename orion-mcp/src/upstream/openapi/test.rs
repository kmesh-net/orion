...
async fn setup() -> (MockServer, Handler) {
    ...
    let client = Client::new(
        &client::Config { resolver_cfg: ResolverConfig::default(), resolver_opts: ResolverOpts::default() },
        None,
    );
    let pi = Arc::new(ProxyInputs {
        ...
        stores: stores.clone(),
        tracer: None,
        metrics: Arc::new(crate::metrics::Metrics::new(metrics::sub_registry(&mut Registry::default()))),
    ...
    let upstream_call_get = UpstreamOpenAPICall { method: "GET".to_string(), path: "/users/{user_id}".to_string() };
    ...
    let upstream_call_post = UpstreamOpenAPICall { method: "POST".to_string(), path: "/users".to_string() };
    ...
    let handler = Handler {
        ...
        backend: SimpleBackend::Opaque(
            String::literal!("dummy"),
            Target::Hostname(parsed.host().unwrap().to_string().into(), parsed.port().unwrap_or(8080)),
        ),
    };
...
async fn test_call_tool_get_simple_success() {
    ...
    let result =
        handler.call_tool("get_user", Some(args.as_object().unwrap().clone()), &IncomingRequestContext::empty()).await;
...
async fn test_call_tool_post_with_body() {
    ...
    let result = handler
        .call_tool("create_user", Some(args.as_object().unwrap().clone()), &IncomingRequestContext::empty())
        .await;
...
async fn test_call_tool_tool_not_found() {
    ...
    let args = json!({});
    let result = handler
        .call_tool("nonexistent_tool", Some(args.as_object().unwrap().clone()), &IncomingRequestContext::empty())
        .await;
...
async fn test_call_tool_invalid_path_param_value() {
    ...
    assert!(result.is_err());
    assert!(
        result.as_ref().unwrap_err().to_string().contains("failed with status 404 Not Found"),
        "{}",
